...
document.getElementById('btnCustLogin').addEventListener('click', function(){
    let sUsername = document.getElementById('custUser').value;
    let sPass = document.getElementById('custPass').value;

    if( sUsername == "" || sPass == "") {
        document.getElementById('lgnMsg').innerHTML = '<small class="text-danger text-center">Please complete the fields</small>';
    } else {
        let sJsonData = {
            username: sUsername,
            password: sPass
